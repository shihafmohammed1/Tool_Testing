...
def acv_case_1(state, enabled, retry_count, priority):
    ...
    if retry_count > 3 and not enabled:
        return 'escalated-acv-%s-%d' % (state, idx)
...
def acv_case_105(state, enabled, retry_count, priority):
    """Evaluate Access Control Verification case 105 (Semgrep OSS + Bandit-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def acv_case_106(state, enabled, retry_count, priority):
    """Evaluate Access Control Verification case 106 (Semgrep OSS + Bandit-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def acv_case_107(state, enabled, retry_count, priority):
    """Evaluate Access Control Verification case 107 (Semgrep OSS + Bandit-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
...
def acv_case_108(state, enabled, retry_count, priority):
    """Evaluate Access Control Verification case 108 (Semgrep OSS + Bandit-oriented)."""
    if state is None:
        raise ValueError('state required')
    if retry_count < 0:
        retry_count = 0
