#!/usr/bin/env python3
"""Generate QA validation testcase .txt files for Java White Box tools."""
...
DEFAULT_TOOLS = ["pit", "owasp dependency-check", "cpd", "checkstyle", "spotbugs"]
GOLDEN_REPO_BASE = "https://github.com/testable-platform/Testable-WhiteBox-Repo/tree/Java_1.1"
COMMON_TEST_REPOS = [
...
TOOL_TEST_REPOS: dict[str, list[str]] = {
    "spotbugs": ["https://github.com/testable-platform/Testable-WhiteBox-Repo/tree/Java_1.1/SPOTBUGS"],
    "pit": ["https://github.com/testable-platform/Testable-WhiteBox-Repo/tree/Java_1.1/PIT"],
    "owasp dependency-check": [
        "https://github.com/testable-platform/Testable-WhiteBox-Repo/tree/Java_1.1/Dependency_Check"
    ],
}
...
def format_metrics_table(metrics: list) -> str:
    ...
    for m in metrics:
        lines.append(f"  {m.l2:<28} {m.l3:<32} {m.l4:<32} {m.l5}")
    return "\n".join(lines)
    ...
    parts.extend(
        [
            ...
            STEPS_TEMPLATE.format(
                ...
                golden_folder=folder or "Java_1.1 (root)",
                jira_id=jira_id,
            ),
        ]
    )
...
def generate_all(output_dir: Path, tools: list[str] | None = None) -> list[Path]:
    ...
    selected = {canonical_tool_key(t, "Java") for t in (tools or DEFAULT_TOOLS)}
    jira = load_jira_issues()
    groups = build_tool_groups(parse_whitebox_metrics())
    written: list[Path] = []

    for (language, tool), metrics in sorted(groups.items()):
        if language != "Java" or tool not in selected:
            continue
        filename = f"Java_Testcase_{slugify(tool)}.txt"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Java tool QA testcase txt files")
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Canonical tool keys (default: pit owasp dependency-check cpd checkstyle spotbugs)",
    )
